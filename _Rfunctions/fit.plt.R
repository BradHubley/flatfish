...
if(missing(ymaxR))ymax<-ifelse(CI,max(pmax(apply(sweep(data.out$sims.list$R,2,FUN='*',data.out$median$q*rl/Iadj), 2, quantile, 1-alpha/2), data.out$data$IR/Iadj)),max(c((data.out$median$R[1:length(data.out$data$iyr)]*data.out$median$q*rl)/Iadj,data.out$data$IR/Iadj)))
else ymax<-ymaxR
plot(data.out$data$iyr, (data.out$median$R[1:length(data.out$data$iyr)]*data.out$median$q*rl)/Iadj, type = 'l', lwd=2, ylim = c(0,ymax), ylab = "", las = 1, xlim = c(min(data.out$data$iyr)-1, max(data.out$data$iyr)+1), mgp = c(0.5, 0.5, 0), xlab = "", tcl = -0.3, asp = 'xy', cex.axis=1.2)
axis(4, lab = F, tcl = -0.3)
if(Iadj!=1)mtext("Survey Recruit Biomass \n (kg/tow)", 2, 2.5, cex = 1.25)
if(Iadj==1)mtext("Survey Recruit Biomass", 2, 3.5, cex = 1.25)
if(CI){
	lines(data.out$data$iyr, apply(sweep(data.out$sims.list$R,2,FUN='*',data.out$median$q*rl/Iadj), 2, quantile, alpha/2), lty = 2)
	lines(data.out$data$iyr, apply(sweep(data.out$sims.list$R,2,FUN='*',data.out$median$q*rl/Iadj), 2, quantile, 1-alpha/2), lty = 2)
}
